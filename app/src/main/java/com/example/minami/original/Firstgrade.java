...
public class Firstgrade extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        List<Question> firstList = new ArrayList();
        mAdapter = new MyListAdapter(getApplicationContext(), R.layout.question_list_item);
        mAdapter.addAll(mlist);
        for (int i = 0; i<mlist.size(); i++){
            if (mlist.get(i).getGrade().equals("1級")){
                firstList.add(mlist.get(i));
            }
        }
        ...
        for(int i = 0; i < EIGO_COUNT; i = i+1) {
            randomNumber = random.nextInt(firstList.size());
            array.add(i, firstList.get(randomNumber));
        }
