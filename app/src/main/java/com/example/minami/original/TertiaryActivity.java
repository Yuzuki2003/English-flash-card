...
public class TertiaryActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        List<Question> tertiaryList = new ArrayList();
        mAdapter = new MyListAdapter(getApplicationContext(), R.layout.question_list_item);
        mAdapter.addAll(mlist);
        for (int i = 0; i<mlist.size(); i++){
            if (mlist.get(i).getGrade().equals("3級")){
                tertiaryList.add(mlist.get(i));
            }
        }
        ...
        for(int i = 0; i < EIGO_COUNT; i = i+1) {
            randomNumber = random.nextInt(tertiaryList.size());
            array.add(i, tertiaryList.get(randomNumber));
        }
