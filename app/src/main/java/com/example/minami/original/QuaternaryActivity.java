...
public class QuaternaryActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        List<Question> quaternaryList = new ArrayList();
        mAdapter = new MyListAdapter(getApplicationContext(), R.layout.question_list_item);
        mAdapter.addAll(mlist);
        for (int i = 0; i<mlist.size(); i++){
            if (mlist.get(i).getGrade().equals("4級")){
                quaternaryList.add(mlist.get(i));
            }
        }
        ...
        for(int i = 0; i < EIGO_COUNT; i = i+1) {
            randomNumber = random.nextInt(quaternaryList.size());
            array.add(i, quaternaryList.get(randomNumber));
        }
