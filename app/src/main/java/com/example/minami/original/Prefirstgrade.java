...
public class Prefirstgrade extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        List<Question> prefirstList = new ArrayList();
        mAdapter = new MyListAdapter(getApplicationContext(), R.layout.question_list_item);
        mAdapter.addAll(mlist);
        for (int i = 0; i<mlist.size(); i++){
            if (mlist.get(i).getGrade().equals("準1級")){
                prefirstList.add(mlist.get(i));
            }
        }
        ...
        for(int i = 0; i < EIGO_COUNT; i = i+1) {
            randomNumber = random.nextInt(prefirstList.size());
            array.add(i, prefirstList.get(randomNumber));
        }
